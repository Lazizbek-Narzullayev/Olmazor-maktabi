            ...
            <div className="max-w-6xl mx-auto space-y-10 animate-fade-in-up">
                <div className="bg-white rounded-[2.5rem] p-10 border border-slate-200 shadow-2xl shadow-indigo-100/20 relative overflow-hidden group">
                    ...
                    <form onSubmit={saveLibraryFile} className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
                        ...
                        <div className="md:col-span-3 bg-slate-50/50 border-2 border-dashed border-slate-200 rounded-[2rem] p-10 flex flex-col items-center justify-center text-center transition-all hover:border-indigo-300 hover:bg-slate-50 group/upload"><FaPlus className="text-4xl text-slate-200 mb-4 group-hover/upload:text-indigo-400 transition-all" /><input type="file" onChange={e => setLibForm({...libForm, file: e.target.files[0]})} required className="hidden" id="fileUpload" /><label htmlFor="fileUpload" className="cursor-pointer"><span className="text-indigo-600 font-black text-sm block mb-1">Faylni tanlash uchun bosing</span><span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">{libForm.file ? libForm.file.name : 'PDF, DOCX yoki PPTX formatda'}</span></label></div>
